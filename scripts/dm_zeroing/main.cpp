...
int main()
{
    ...
    J4340 j1(can, {
                 tbl["joint_1"]["can_id"].value<uint16_t>().value(),
                 tbl["joint_1"]["master_id"].value<uint16_t>().value(),
                 MITMode{0, 0}
             });
    J10010L j2(can, {
                   tbl["joint_2"]["can_id"].value<uint16_t>().value(),
                   tbl["joint_2"]["master_id"].value<uint16_t>().value(), MITMode{0, 0}
               });
    J8009 j3(can, {
                 tbl["joint_3"]["can_id"].value<uint16_t>().value(),
                 tbl["joint_3"]["master_id"].value<uint16_t>().value(), MITMode{0, 0}
             });
    J4310 j5(can, {
                 tbl["joint_5"]["can_id"].value<uint16_t>().value(),
                 tbl["joint_5"]["master_id"].value<uint16_t>().value(), MITMode{0, 0}
             });
    ...
    auto t = std::jthread([&](const std::stop_token& stop_token)
    {
        ...
        {
            std::cout << std::format("j1: {}, j2: {}, j3: {}, j5: {}", j1.getStatus().position,
                                     j2.getStatus().position,
                                     j3.getStatus().position, j5.getStatus().position);
