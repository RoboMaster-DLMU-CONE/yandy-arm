...
static void register_asset(rerun::RecordingStream &rec,
  ...
  auto asset = rerun::Asset3D::from_file_contents(
      bytes, ext == ".stl" ? std::optional(rerun::components::MediaType::stl())
...
int main() {
  ...
  const std::string energy_unit_stl =
      YANDY_CONFIG_PATH "urdf/meshes/energy_unit.stl";
  constexpr double viz_dt = 0.016; // ~60Hz 可视化刷新率

  // ---- Init Rerun ----
  auto robot_tbl = toml::parse_file(YANDY_ROBOT_CONFIG);
  std::string rerun_ip = "127.0.0.1";
  int rerun_port = 9876;

  if (robot_tbl["simulate_rerun"].is_table()) {
    rerun_ip = robot_tbl["simulate_rerun"]["ip"].value_or("127.0.0.1");
    rerun_port = robot_tbl["simulate_rerun"]["port"].value_or(9876);
  }

  diag_logger->info("Connecting to Rerun at {}:{}", rerun_ip, rerun_port);

  auto rec = rerun::RecordingStream("robomaster_arm_sim");
  rec.connect_grpc(
         std::format("rerun+http://{}:{}/proxy", rerun_ip, rerun_port))
      .exit_on_failure();
  ...
  pinocchio::urdf::buildGeom(
      viz_model, urdf_file, pinocchio::VISUAL, geom_model,
  ...
  const auto resolve_mesh_path =
      [&](const std::string &raw_path) -> std::filesystem::path {
    if (raw_path.empty())
      return {};
  ...
  for (const auto &geom : geom_model.geometryObjects) {
    register_asset(rec, viz_root + "/" + geom.name,
                   resolve_mesh_path(geom.meshPath));
  }
  ...
  rec.log_static("world/target_ghost", rerun::Points3D({{0.0f, 0.0f, 0.0f}})
                                           .with_radii({0.05f})
                                           .with_colors({0x00FF00FF}));
  rec.log_static("world/ee_actual", rerun::Points3D({{0.0f, 0.0f, 0.0f}})
                                        .with_radii({0.03f})
  ...
  while (g_running.load(std::memory_order_relaxed)) {
    ...
    // A. 更新机器人几何体位姿
    pinocchio::updateGeometryPlacements(viz_model, viz_data, geom_model,
                                        geom_data, vd.q);
    ...
    for (pinocchio::GeomIndex i = 0; i < geom_model.ngeoms; ++i) {
      ...
      rec.log_static(
          ...
          rerun::Transform3D::from_translation_rotation(
              rerun::components::Translation3D((float)t.x(), (float)t.y(),
                                               (float)t.z()),
    ...
    // C. 实际末端点 (红色球) — 只更新 Transform
    {
      ...
      rec.log_static("world/ee_actual",
                     rerun::Transform3D(rerun::components::Translation3D(
                         (float)t.x(), (float)t.y(), (float)t.z())));
    }
    ...
    rec.log_static("state",
                   rerun::TextDocument(std::string(format_as(vd.state))));
    ...
    // 输出 target 位姿的数值（XYZ + RPY in degrees） 调试用
    {
      ...
      Eigen::Vector3d rpy =
          R.eulerAngles(0, 1, 2); // roll, pitch, yaw (radians)
