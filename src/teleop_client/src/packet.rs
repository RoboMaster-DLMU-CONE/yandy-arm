...
pub struct YandyControlPack {
    ...
    pub y: f32,            // 末端位置 Y (m)
    pub z: f32,            // 末端位置 Z (m)
    pub ee_qw: f32,        // 末端姿态四元数 W
    pub ee_qx: f32,        // 末端姿态四元数 X
    pub ee_qy: f32,        // 末端姿态四元数 Y
    pub ee_qz: f32,        // 末端姿态四元数 Z
...
// 编译期大小验证 — 必须与 C++ Types.hpp 中的 YandyControlPack 一致 (81 字节)
const _: () = assert!(
    core::mem::size_of::<YandyControlPack>() == 81,
    "YandyControlPack size mismatch! Must match C++ Types.hpp (81 bytes)."
);
...
impl PacketSerializer {
    ...
    pub fn serialize(&mut self, packet: &YandyControlPack) -> Vec<u8> {
        ...
        // Data Size (Little Endian) at offset 1-2
        let data_size_u16 = data_size as u16;
        buffer[1] = (data_size_u16 & 0xFF) as u8;
        buffer[2] = ((data_size_u16 >> 8) & 0xFF) as u8;

        // Seq at offset 3
        buffer[3] = self.sequence;

        // Header CRC8 (over first 4 bytes) at offset 4
        buffer[4] = crc8::calc(&buffer[0..4]);

        // CMD (Little Endian) at offset 5-6
        buffer[5] = (cmd_id & 0xFF) as u8;
        buffer[6] = ((cmd_id >> 8) & 0xFF) as u8;
...
mod crc8 {
    // CRC-8/MAXIM: poly=0x31, init=0x00 (after reflection), reflected input/output
    // Using lookup table for performance - matches cppcrc CRC8::MAXIM
    ...
    pub fn calc(data: &[u8]) -> u8 {
        let mut crc = 0xFFu8; // init value for CRC-8 with poly=0x31, init=0xFF, reflected
        for &byte in data {
            let index = (byte ^ crc) as usize;
...
mod crc16 {
    // CRC-16/MCRF4XX: poly=0x1021, init=0xFFFF, reflected input/output, xor_out=0x0000
    // Using lookup table for performance and accuracy - matches cppcrc CRC16::MCRF4XX
    ...
    pub fn calc(data: &[u8]) -> u16 {
        let mut crc = 0xFFFFu16; // Init value for CRC-16/MCRF4XX (reverse_bits_u16(0xFFFF) = 0xFFFF)
        for &byte in data {
            let index = ((byte as u16) ^ crc) as usize;
            crc = CRC16_TABLE[index & 0xFF] ^ (crc >> 8);
        }
        crc
    }
}
