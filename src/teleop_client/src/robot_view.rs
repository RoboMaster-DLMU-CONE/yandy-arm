...
impl RobotView {
    pub fn new(window: &mut Window, urdf_path: &str) -> Self {
        ...
        let base_link = robot
            .links
            .iter()
            .find(|l| l.name == robot.name || l.name == "base_link")
        ...
        if let Some(link) = base_link {
            ...
            let link_map: HashMap<String, &urdf_rs::Link> =
                robot.links.iter().map(|l| (l.name.clone(), l)).collect();
            ...
            match geom {
                urdf_rs::Geometry::Mesh { filename, scale } => {
                    ...
                    if let Some(mesh) = load_stl(&mesh_path) {
                        ...
                        let translation =
                            na::Translation3::new(xyz[0] as f32, xyz[1] as f32, xyz[2] as f32);
...
fn load_stl(path: &Path) -> Option<Rc<RefCell<Mesh>>> {
    ...
    Some(Rc::new(RefCell::new(Mesh::new(
        coords, indices, None, None, false,
    ))))
}
