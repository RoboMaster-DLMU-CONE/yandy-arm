...
* **默认层 (Default)**：机械臂移动、云台瞄准。
* **功能层 (Function)**：单按键触发模式切换。
* **调试层 (Debug)**：`Ctrl` + 按键触发数据修正。

#### **A. 核心功能区 (单键触发)**

| 键位    | 对应 Enum             | 助记符 / 理由                                                             |
|:------|:--------------------|:---------------------------------------------------------------------|
| **F** | `CMD_SWITCH_FETCH`  | **F**etch (取矿)。手指最顺手的位置。                                             |
| **R** | `CMD_SWITCH_STORE`  | **R**epository (存矿)。在 F 上方，顺手。                                       |
| **G** | `CMD_SWITCH_ENABLE` | **G**o / **G**reen (启用/激活)。相当于 Start 键。                              |
| **C** | `CMD_SWITCH_GRIP`   | **C**law (夹爪)。手动开关夹爪，C 键在拇指控制区。                                      |
| **Z** | `CMD_RESET`         | **Z**ero (归零/复位)。左下角，防止误触。                                           |
| **B** | `CMD_ERROR`         | **B**reak / **B**lock (急停)。**B** 是这组键里最远的，作为急停键，需要有意识地去按，或者大拇指大跨度去按。 |

#### **B. 调试修正区 (组合键: Ctrl + Key)**

当视觉识别错误或存弹量计数错误时使用。

| 组合键          | 对应 Enum                 | 助记符 / 理由                     |
|:-------------|:------------------------|:-----------------------------|
| **Ctrl + F** | `CMD_DEBUG_TOGGLE_HELD` | 强制修改“持有状态”。(对应 F 键的 Fetch)   |
| **Ctrl + E** | `CMD_DEBUG_INC_STORE`   | **E**xtra (增加库存)。E 在右边，代表增加。 |
| **Ctrl + Q** | `CMD_DEBUG_DEC_STORE`   | **Q**uit (减少库存)。Q 在左边，代表减少。  |
